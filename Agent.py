...
class Agent:
    ...
    def Solve(self, problem):
        ...
        image_A_data = problem_figures['A']
        image_B_data = problem_figures['B']
        image_C_data = problem_figures['C']
        transformations_AB = \
            Transform.generate_transforms_data(image_A_data, image_B_data)
        transformations_AC = \
            Transform.generate_transforms_data(image_B_data, image_C_data)
        for transform_AB in transformations_AB.itervalues():
            for transform_AC in transformations_AC.itervalues():
                for solution_name, solution_image_data in \
                        solution_figures.iteritems():
                    image_AB_to_C_match = transform_AB.apply_and_compare(
                        image_C_data, solution_image_data)
                    image_AB_to_C_match_difference = \
                        math.fabs(image_AB_to_C_match - transform_AB.match)
                    image_AC_to_B_match = transform_AC.apply_and_compare(
                        image_B_data, solution_image_data)
                    image_AC_to_B_match_difference = \
                        math.fabs(image_AC_to_B_match - transform_AC.match)
                    total_match_difference = image_AB_to_C_match_difference \
                                             + image_AC_to_B_match_difference
                    if total_match_difference < closest_match['difference']:
                        closest_match['name'] = solution_name
                        closest_match['difference'] = total_match_difference
